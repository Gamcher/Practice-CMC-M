...
void executeSequence(char **, int, int, int*);
void appendWord(char **, int, char*);
int lenInt(int*);
void checkLogic(char**);
void cutNextSequence(int, char **);
void myHandlerCommand(char **, int, int, int *);
void checkInOut(char **, int *, int *, int *);

//если будет тест про скобки в конвейере, то буяним, и говорим, что работает не так, и там написано, что должно работать так
    ...
    while(c != EOF){
        ...
        if(arWords != NULL){
            if(arWords[0] != NULL){
                ...
                int fdo1, fdo2;
                fdo1 = fdo2 = -1;
                checkInOut(arWords, &fdo1, &fdo2, &status);
                myHandlerCommand(arWords, fdo1, fdo2, &status);
                freeStr(arWords);
            }
...
void executeCommand(char ** arrWords, int fdoI1, int fdoI2, int * status, int phone){
...
void executeSequence(char ** arWords, int fdo1, int fdo2, int *status){ //все fd отцов передаются, и они важнее
    int checkSemicolon, checkPhone, j = 0;
    int fdInteractO1, fdInteractO2;
    if((fdo1 >= 0)||(fdo2 >= 0)){ //важнее то, что в скобках
        fdInteractO1 = fdo1;
        fdInteractO2 = fdo2;
    }
    else checkInOut(arWords, &fdInteractO1, &fdInteractO2, status);
    do{ 
        ...
        executeCommand(commandArray, fdInteractO1, fdInteractO2, status, checkPhone);
        if (commandArray != NULL) free(commandArray); 
...
void myHandlerCommand(char ** arWords, int fdo1, int fdo2, int * status){
    int index = 0;
    int checkBracket = 0;
    ...
    while(arWords[index] != NULL){ //обработка скобок
        ...
        if(((!strcmp(arWords[index], "||"))||(!strcmp(arWords[index], "&&")))&&(bLeftI == 0)){
            ...
            executeSequence(commandArray, fdo1, fdo2, status); //надо передавать более крутых наверх
            if (commandArray != NULL) free(commandArray);
        ...
        if(!strcmp(arWords[index], "(")){
            ...
            pid_t pid;
            //жесткая проверка на нахождение >>, <, > за скобками. Если есть - забираем их, если нет - то всё ок
            //нужна переменная fdInteractO1, fdInteractO2, отвечающие за внешние
            int fdInteractO1, fdInteractO2;
            if((fdo1 >= 0)||(fdo2 >= 0)){
                fdInteractO1 = fdo1;
                fdInteractO2 = fdo2;
            ...
            if((pid = fork()) > 0){
                ...
                myHandlerCommand(commandArray, fdInteractO1, fdInteractO2, status);
                exit(*status);
            }
            free(commandArray);
    ...
    free(bracketLeft);
    if (arWords[0] != NULL){ 
        if((strcmp(arWords[0], "YES") != 0)&&(strcmp(arWords[0], "NO") != 0)) executeSequence(arWords, fdo1, fdo2, status);
...
void checkInOut(char ** arrWords, int * fdo1, int * fdo2, int * status){
    int index = 0;
    int countBrackets = 0;
    while(arrWords[index]!= NULL){
        if(!strcmp(arrWords[index], "(")){
             countBrackets++;
             index++; 
             continue;
        }
        ...
        if(countBrackets == 0){
            if(!strcmp(arrWords[index], ">")){
                *fdo1 = open(arrWords[index+1], O_WRONLY | O_CREAT | O_TRUNC, 0666);
                if (*fdo1 < 0){
                    fprintf(stderr, "Некорректное имя файла для перенаправления вывода\n");
                    _exit(*status);
                }
                deleteNWords(index, arrWords, 2);
                continue;
            }
            if(!strcmp(arrWords[index], ">>")){
                *fdo2 = open(arrWords[index+1], O_WRONLY | O_CREAT | O_APPEND, 0600);
                if (*fdo2 < 0){
                    fprintf(stderr, "Некорректное имя файла для перенаправления вывода\n");
                    _exit(*status);
                }
