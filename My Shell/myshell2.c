            ...
            if(arWords[0] != NULL){
                ...
                while(arWords[index] != NULL){ //обработка скобок
                    ...
                    if(!strcmp(arWords[index], "(")){
                        ...
                        bLeftI--;
                        printf("Size minimum\n");
                        printf("kkkjdj - %d", bLeftI);
                        int indexClose = bracketLeft[bLeftI];
                        printf("kkkjj - %d", indexClose);
                        bracketLeft[bLeftI] = -1;
                        commandArray = dupNWords(indexClose+1, commandArray, index-indexClose-1); //дублирую команду
                        printArr(commandArray);
                        executeSequence(commandArray, &status);
                        printf("Zzzzzzzzzzzz\n");
                        deleteNWords(index, arWords, index-indexClose+1);
                        index = indexClose;
                        if(status == 0)
                            appendWord(arWords, index, "YES\0");
                        else appendWord(arWords, index, "NO\0");
                        checkLogic(arWords); //описать функцию, которая проверяет ||, &&
                    }
                ...
                if (arWords[0] != NULL){ 
                    if(strcmp(arWords[0], "YES") != 0) executeSequence(arWords, &status);
                }
